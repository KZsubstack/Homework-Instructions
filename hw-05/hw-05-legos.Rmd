...
The three most common first name of purchasers are Jackson, Jacob, and Joseph.
...
Star Wars, Nexo Knights, and Gear.

3.  Among the most common theme of Lego sets purchased, what is the most common subtheme?

The Force Awakens is the most common theme of Lego sets purchased
...
18 and under (30), 19–25 (129), 26–35 (183), 36–50 (216), and 51 and over (62)
...
```{r q6, eval = TRUE}
lego_sales %>%
  mutate(spending = quantity * us_price) %>%
  group_by(age_group) %>%
  summarise(total_spent = sum(spending)) %>%
...
Age Group 36-50 with their total spent as $9532.87
...
The Star Wars theme has made the most money for Lego at $4447.83 
...
lego_sales %>%
  ...
  mutate(
    area_code = str_sub(phone_number, 1, 3),
    spending = quantity * us_price
  ) %>%
  group_by(area_code) %>%
  summarise(total_spent = sum(spending)) %>%
  arrange(desc(total_spent)) %>%
...
The Area code with the most total spent is NA at 3992.64 dollars. However, if we are only talking about available area codes it is 956 with 719.96 dollars. 
...
My research question is: Does average spending on the  transaction level differ across different age groups?

we would need three variables to answer this: age_group, quantity, us_price

The plot to use is bar chart because its code for comparing a numeric value across different categories

The 36–50 age group spends the most per transaction on average, while the 18 and under group spends the least. This is interesting because although the 36–50 group also leads in total purchases, the average transaction result suggests that older customers tend to select more expensive or larger Lego sets per purchase not just that they buy more frequently.
...
lego_sales %>%
  ...
  ggplot(aes(x = age_group, y = avg_spending, fill = age_group)) +
  geom_col() +
  labs(
    title = "Average Lego Spending per Transaction by Age Group",
    x = "Age Group",
