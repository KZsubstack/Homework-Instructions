...
we choose the median instead of the mean to describe the typical income of a group of people because the median is more resistant to skewed data or outliers giving a better representation.
...
Based on the shape of the histogram, the summary statistics most useful for describing the distribution are the median and the quartiles (IQR). While the mean can help us see which way the shape is skewed by comparing it to the median, it is easily pulled away by extreme values. In this case, because the mean ($40,151$) is significantly higher than the median ($36,000$), it shows that the distribution is actually right-skewed, meaning it leans more toward the minimum values with a long tail stretching toward the maximum values.The standard deviation also helps describe the spread of the distribution; a larger standard deviation indicates that the data is more spread out, while a smaller one would mean the values are more clustered around the center. Finally, the quartiles show us that the middle $50\%$ of majors earn between $\$33,000$ (Q1) and $\$45,000$ (Q3). This concentration at the lower end, combined with the unusual observation of an outlier at $\$110,000$, confirms that the distribution is skewed to the right.
...
```
Exercise 9 written interpretation:

There is a negative association between proportion of women and median income, meaning majors with more women tend to have lower median earnings. STEM majors cluster toward lower proportions of women and higher median incomes, while non-STEM majors tend to have higher proportions of women and lower median incomes. This suggests that the gender gap in earnings may be partly explained by the types of majors women choose rather than earnings differences within the same major.
...
ggplot(data = college_recent_grads, 
       aes(x = sharewomen, y = reorder(major_category, sharewomen))) +
  geom_boxplot(fill = "green", color = "black") +
  labs(
    x = "Proportion of Women",
    y = "Major Category",
    title = "Distribution of Women in Majors by Category",
  )
```

**Your findings (3-4 sentences):**
Health, Education, interdisciplinary and Psychology & Social Work have the largest proportion of women with each of these majors having over a 75% proportion of Women. Engineering and industrial arts & Consumer services have the least proportion of women on average, Industrial Arts & Consumer Services and Agriculture & Natural Resources have much "longer" boxes so these majors can be male or female dominated. Biology & Life Science, communications & Journalism, and Psychology & Social work have the tightest ranges but they all sit above 50%.
...
- **Code chunk not running** → Remove `eval = TRUE` from chunk options
