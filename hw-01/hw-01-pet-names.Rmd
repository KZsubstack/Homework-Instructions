...
There are 52,519 pets included in the dataset. 
...
There are 7 variables for each pet the license_issue_date, license_number, animal_name, species, primary_breed, secondary_breed, and zip_code
...
We ignore NA because that means the pet name is missing, that missing name could be anything it's not a real name top three names are Lucy (439), Charlie (387), and Luna (355) Code is below:

```{r countrepeat, include = TRUE, eval=TRUE}
seattlepets %>%
  count(animal_name, sort = TRUE)
```
...
arrange(species, desc(n)) groups the different species together in their own sections, with their names ranked by frequecy within each species adding the desc(n) makes it so it orders from most to least, which is what we want to see the top name within each species.

```{r species-names-top5 question duplicate, eval=TRUE}
seattlepets %>% 
  group_by(species) %>%
  count(animal_name, sort = TRUE) %>% 
  slice_max(n, n = 5) %>% 
...
The names below the line are more common for cats than dogs. The names above the line are more common for dogs.
...
Yes the relationship between the two variables have a positive relationship this means, in the context of the data, names that have a higher proportion among dogs also tend to have a higher proportion among cats. So if name is popular with cats it will likely be popular with dogs too or vice versa.
