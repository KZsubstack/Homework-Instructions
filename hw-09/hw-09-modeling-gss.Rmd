...
Agree: 1159. Not agree: 226. NA: 1482.
...
Conservative: 928. Moderate: 1032. Liberal: 796. NA: 111.
...
The frame has 1328 rows after dropping NAs.
...
Training set: 996 rows. Testing set: 332 rows.
...
Fold-level accuracy sits between 0.829 and 0.835, with standard error of 0.001. Accuracy stays stable across folds. Fold-level ROC AUC swings wider, from 0.578 to 0.721, with standard error of 0.026. Average accuracy across folds: 0.832. Average ROC AUC across folds: 0.633.
...
set.seed(345)
gss16_folds_2 <- vfold_cv(gss16_train, v = 5)
...
Average accuracy across folds: 0.831. Average ROC AUC across folds: 0.629.
...
Model 1 ROC AUC is 0.633. Model 2 ROC AUC is 0.629. Model 1 edges Model 2 by 0.004 on the training cross-validation. The gap sits inside both standard errors (0.026 and 0.021), so the lead is small. Model 1 wins on training CV, barely.
...
```{r predict-test, eval = TRUE}
pred_1 <- predict(gss16_fit_1, gss16_test, type = "prob") %>%
  bind_cols(gss16_test %>% select(advfront))

pred_2 <- predict(gss16_fit_2, gss16_test, type = "prob") %>%
  bind_cols(gss16_test %>% select(advfront))

roc_auc(pred_1, truth = advfront, .pred_Agree)
roc_auc(pred_2, truth = advfront, .pred_Agree)
```

```{r roc-plot, eval = TRUE, fig.cap = "ROC curves on the test set."}
roc_1 <- roc_curve(pred_1, truth = advfront, .pred_Agree) %>%
  mutate(model = "Model 1 (with wrkstat)")
roc_2 <- roc_curve(pred_2, truth = advfront, .pred_Agree) %>%
  mutate(model = "Model 2 (no wrkstat)")

bind_rows(roc_1, roc_2) %>%
...
Test ROC AUC for Model 1: 0.633. Test ROC AUC for Model 2: 0.648. The ranking flips. Model 2 wins on the test set by 0.015. The ROC plot confirms this: Model 2 sits above Model 1 across most of the false-positive-rate range. The answer to Q10 does not hold on the test data. Adding wrkstat helped slightly on training folds and hurt on the held-out test set. The extra predictor added noise the simpler model avoids. Pick Model 2.
...
```{r harass-subset, eval = TRUE}
gss16_h <- gss16 %>%
  filter(harass5 %in% c("Yes", "No")) %>%
  mutate(harass5 = factor(harass5, levels = c("Yes", "No")))

gss16_h %>% count(harass5)
```

Yes: 237 responses. No: 1136 responses.
...
Treat the 1373 Yes and No responses as your sample. Draw a new sample of 1373 responses from the original sample with replacement. Compute the proportion of Yes responses in the resample. Repeat the draw 1000 times. The 1000 proportions approximate the sampling distribution of the proportion. Take the middle 95 percent of those proportions using the 2.5th and 97.5th percentiles to build a 95 percent confidence interval for the population proportion of Americans harassed at work over the past five years.
...
```{r boot-viz, eval = TRUE, fig.cap = "Bootstrap distribution of the proportion harassed with 95 percent CI shaded."}
boot_dist %>%
  visualize() +
  shade_confidence_interval(endpoints = ci_95) +
  labs(
    title = "Bootstrap Distribution of Proportion Harassed",
    x = "Proportion Yes",
    y = "Count"
  )
```

**Interpretation:** The 95 percent bootstrap CI runs from 0.154 to 0.194. You are 95 percent confident the true proportion of Americans harassed by their superiors or co-workers at their job over the past five years falls between 15.4 percent and 19.4 percent. The point estimate of 17.3 percent sits at the center of a roughly symmetric bootstrap distribution.
...
A 90 percent confidence interval would be narrower than the 95 percent interval. Lower confidence requires capturing less of the bootstrap distribution, so the interval needs less width to hold the central 90 percent of resampled proportions rather than the central 95 percent. Verification below.
...
The 90 percent CI runs from 0.157 to 0.190, width 0.033, compared with the 95 percent CI width of 0.040.
